...
@Component({
  selector: 'app-tipos-equipos-remolques',
  templateUrl: './tipos-equipos-remolques.component.html',
  styleUrls: ['./tipos-equipos-remolques.component.css']
})
export class TiposEquiposRemolquesComponent implements OnInit {
  ...
  private columnasDefault: Columna[] = [
    ...
    { nombre: 'Prefijo', targetId: 'prefijo', tipo: 'texto', aligment: 'right' },
  ...
  cargarComponent() {
    if (this.router.url.endsWith('tiposEquiposRemolques')) {
      this.rutaComponent = this.router.url;
      this.serviceComponent.setConfiguracionComponent({
        header: {
          titulo: 'Tipos de Equipos de Remolques',
          opciones: [
            {
  ...
  eliminarItem(item: TipoEquipoRemolque) {
    this.sweetService.sweet_confirmacion(
      '¡Advertencia!',
      `¿Desea eliminar el Tipo de Equipo de Remolque ${item.descripcion}?`,
      'warning').then(res => {
        if (res.isConfirmed) {
