import { Municipio } from './../../data/models/municipio';
...
export class MunicipiosComponent implements OnInit {
  ...
  private columnasDefault: Columna[] = [
    ...
    { nombre: 'Cód. de Municipio', targetId: 'codMunicipio', tipo: 'texto', aligment: 'left' },
    { nombre: 'Nombre de Municipio', targetId: 'nombreMunicipio', tipo: 'texto', aligment: 'left' },
    { nombre: 'Id Departamento', targetId: 'idDepartamento', tipo: 'texto', aligment: 'left' },
    { nombre: 'Fecha de Creación', targetId: 'fechaCreacion', tipo: 'fecha', aligment: 'right' },
    //{ nombre: '', targetId: '', texto: true, aligment:'center'},
  ];

  constructor(private municipioService: MunicipioService, private router: Router, private sweetService: SweetService) { }

  ngOnInit(): void {
    //Esperamos a que el Service se inicialice
    this.municipioService.getCargando().pipe(
  ...
  cargarComponent() {
    ...
    if (this.router.url.endsWith('municipios')) {
      ...
      this.municipioService.setColumnas(this.columnasDefault);
      ...
      */
      this.municipioService.setMenuOpcionesTabla([
  ...
  cargarPagina(noPagina: number, filtroNombre: string = "") {
    ...
    this.municipioService.cargarPagina(filtroNombre).pipe(
  ...
  eliminarItem(municipio: Municipio) {
    this.sweetService.sweet_confirmacion(
      '¡Advertencia!',
      `¿Desea eliminar el Municipio ${municipio.nombreMunicipio}?`,
      'warning').then(res => {
        if (res.isConfirmed) {
          this.municipioService.eliminar(municipio.id).pipe(first()).subscribe(res => {
  ...
  getColumnas() {
    return this.municipioService.getColumnas();
  }
  ...
  //Carga el paginador 
  getPaginador() {
    return this.municipioService.getPaginador();
  }
  ...
  getMenuOpcionesTabla() {
    return this.municipioService.getMenuOpcionesTabla();
  }
