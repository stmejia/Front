...
export class UbicacionesComponent implements OnInit {
  ...
  private columnasDefault: Columna[] = [
    ...
    { nombre: 'Es puerto', targetId: 'esPuerto', tipo: 'boolean', aligment: 'center' },
    //{ nombre: 'Lugar', targetId: 'lugar', tipo: 'texto', aligment: 'left' },
    { nombre: 'Cód. Postal', targetId: 'codigoPostal', tipo: 'texto', aligment: 'left' },
  ...
  cargarComponent() {
    if (this.router.url.endsWith('ubicaciones')) {
  ...
  eliminarItem(item: Ubicacion) {
    this.sweetService.sweet_confirmacion(
      '¡Advertencia!',
      `¿Desea eliminar el pais ${item.id}?`,
      'warning').then(res => {
        if (res.isConfirmed) {
