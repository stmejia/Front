...
export class TiposVehiculosComponent implements OnInit {
  ...
  constructor(private serviceComponent: TipoVehiculoService, private sweetService: SweetService,
  ...
  cargarComponent() {
    if (this.router.url.endsWith('tiposVehiculos')) {
      this.rutaComponent = this.router.url;
      this.serviceComponent.setConfiguracionComponent({
        header: {
          titulo: 'Tipos de Vehiculos',
          opciones: [
            {
  ...
  eliminarItem(item: TipoVehiculo) {
    this.sweetService.sweet_confirmacion(
      '¡Advertencia!',
      `¿Desea eliminar el Tipo de Vehiculo ${item.descripcion}?`,
      'warning').then(res => {
        if (res.isConfirmed) {
