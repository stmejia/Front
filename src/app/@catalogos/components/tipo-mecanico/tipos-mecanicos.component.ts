import { TipoMecanico } from './../../data/models/tipoMecanico';
...
@Component({
  selector: 'app-tipos-mecanicos',
  templateUrl: './tipos-mecanicos.component.html',
  styleUrls: ['./tipos-mecanicos.component.css']
})
export class TiposMecanicosComponent implements OnInit {
  ...
  private columnasDefault: Columna[] = [
    ...
    { nombre: 'Especialidad', targetId: 'especialidad', tipo: 'texto', aligment: 'left' },
  ...
  cargarComponent() {
    if (this.router.url.endsWith('tiposMecanicos')) {
      this.rutaComponent = this.router.url;
      this.serviceComponent.setConfiguracionComponent({
        header: {
          titulo: 'Tipos de Mecanicos',
          opciones: [
            {
  ...
  cargarPagina(noPagina: number, filtroCodigo = "") {
    ...
    this.serviceComponent.cargarPagina(noPagina, filtroCodigo).pipe(
  ...
  eliminarItem(item: TipoMecanico) {
    this.sweetService.sweet_confirmacion(
      '¡Advertencia!',
      `¿Desea eliminar el Tipo de Mecánico ${item.nombre}?`,
      'warning').then(res => {
        if (res.isConfirmed) {
