import { TipoProveedor } from './../../data/models/tipoProveedor';
...
export class TiposProveedoresComponent implements OnInit {
  ...
  constructor(private serviceComponent: TipoProveedorService, private sweetService: SweetService,
  ...
  cargarComponent() {
    if (this.router.url.endsWith('tipoProveedores')) {
  ...
  eliminarItem(item: TipoProveedor) {
    this.sweetService.sweet_confirmacion(
      '¡Advertencia!',
      `¿Desea eliminar el Tipo de Proveedor ${item.descripcion}?`,
      'warning').then(res => {
        if (res.isConfirmed) {
