...
import { Piloto } from '../../data/models/piloto';
...
export class PilotosComponent implements OnInit {
  ...
  private columnasDefault: Columna[] = [
    { nombre: 'settings', targetId: '', tipo: 'opcion', aligment: 'left' },
    ...
    { nombre: 'Fecha de Ingreso', targetId: 'fechaIngreso', tipo: 'fecha', aligment: 'center' },
    { nombre: 'Fecha de Baja', targetId: 'fechaBaja', tipo: 'fecha', aligment: 'center' },
  ...
  cargarComponent() {
    if (this.router.url.endsWith('pilotos')) {
  ...
  eliminarItem(item: Piloto) {
    this.sweetService.sweet_confirmacion(
      '¡Advertencia!',
      `¿Desea eliminar el piloto ${item.id}?`,
      'warning').then(res => {
        if (res.isConfirmed) {
