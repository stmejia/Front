...
import { TipoGeneradorService } from './../../data/services/tipo-generador.service';
...
@Component({
  selector: 'app-tipos-generador',
  templateUrl: './tipos-generador.component.html',
  styleUrls: ['./tipos-generador.component.css']
})
export class TiposGeneradorComponent implements OnInit {
  ...
  private columnasDefault: Columna[] = [
    ...
    { nombre: 'Prefijo', targetId: 'prefijo', tipo: 'texto', aligment: 'left' }, 
    { nombre: 'Fecha de Creación', targetId: 'fechaCreacion', tipo: 'fecha', aligment: 'center' }
  ];

  constructor(private serviceComponent: TipoGeneradorService, private sweetService: SweetService,
  ...
  cargarComponent() {
    if (this.router.url.endsWith('tiposGeneradores')) {
      this.rutaComponent = this.router.url;
      this.serviceComponent.setConfiguracionComponent({
        header: {
          titulo: 'Tipos de Generadores',
          opciones: [
            {
  ...
  cargarPagina(noPagina: number, filtroCodigo = "") {
  ...
  eliminarItem(item: TipoGenerador) {
    this.sweetService.sweet_confirmacion(
      '¡Advertencia!',
      `¿Desea eliminar el Tipo de Generador ${item.descripcion}?`,
      'warning').then(res => {
        if (res.isConfirmed) {
