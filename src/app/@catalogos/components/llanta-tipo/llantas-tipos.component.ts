import { LlantaTipo } from './../../data/models/llantaTipo';
...
@Component({
  selector: 'app-llantas-tipos',
  templateUrl: './llantas-tipos.component.html',
  styleUrls: ['./llantas-tipos.component.css']
})
export class LlantasTiposComponent implements OnInit {
  ...
  private columnasDefault: Columna[] = [
    ...
    //{ nombre: 'Proposito', targetId: 'proposito', tipo: 'texto', aligment: 'left' },
    { nombre: 'Fecha de Creación', targetId: 'fechaCreacion', tipo: 'fecha', aligment: 'center' }
  ];

  constructor(private serviceComponent: LlantaTiposService, private sweetService: SweetService,
  ...
  cargarComponent() {
    if (this.router.url.endsWith('llantasTipos')) {
      this.rutaComponent = this.router.url;
      this.serviceComponent.setConfiguracionComponent({
        header: {
          titulo: 'Tipos de Llantas',
          opciones: [
            {
  ...
  eliminarItem(item: LlantaTipo) {
    this.sweetService.sweet_confirmacion(
      '¡Advertencia!',
      `¿Desea eliminar el Tipo de Llanta ${item.descripcion}?`,
      'warning').then(res => {
        if (res.isConfirmed) {
