...
export class MedidaService {
  ...
  /* ----- ----- Variables Genericas ----- ----- */
  private urlEndPoint: string = environment.UrlAguilaApi + "/api/medidas";
  ...
  private listaDatos = new BehaviorSubject<Medida[]>([]);
  /* ----- ----- Fin ----- ----- */
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      first(),
      map((res: any) => res.aguilaData as Medida),
      catchError((e) => {
  ...
  setDatos(lista: Medida[]) {
    this.listaDatos.next(lista);
  }

  getDatos(): Observable<Medida[]> {
    ...
    if (this.validarPermiso('Consultar')) {
      this.http.get(this.urlEndPoint +
        `?PageNumber=${noPagina}&idEmpresa=${idEmpresa}`).pipe(
          ...
          map((response: any) => response.aguilaData as Medida[]),
