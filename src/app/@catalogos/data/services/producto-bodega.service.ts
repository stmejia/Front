...
export class ProductoBodegaService {
  ...
  private urlEndPoint: string = environment.UrlAguilaApi + "/api/invProductoBodega";
  ...
  private listaDatos = new BehaviorSubject<ProductoBodega[]>([]);
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      first(),
      map((res: any) => res.aguilaData as ProductoBodega),
      catchError((e) => {
  ...
  getXFiltros(filtros: QueryFilter[]) {
    if (this.validarPermiso('Consultar')) {
      ...
      return this.http.get(this.urlEndPoint + filter).pipe(
        first(),
        map((response: any) => response.aguilaData as ProductoBodega[]),
        catchError((e) => {
          this.sweetService.sweet_Error(e);
          return [];
        }));
  ...
  getEstacionesTrabajo() {
    return this.configService.getEstacionesTrabajo();
  }
