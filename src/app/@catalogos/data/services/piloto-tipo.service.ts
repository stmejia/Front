...
import { PilotoTipo } from '../models/pilotoTipo';

@Injectable({
  providedIn: 'root'
})
export class PilotoTipoService {

  private urlEndPoint: string = environment.UrlAguilaApi + "/api/pilotosTipos";
  ...
  private listaDatos = new BehaviorSubject<PilotoTipo[]>([]);
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      map((response: any) => response.aguilaData as PilotoTipo),
      catchError((e) => {
  ...
  setDatos(lista: PilotoTipo[]) {
    this.listaDatos.next(lista);
  }

  getDatos(cargar: boolean = false): Observable<PilotoTipo[]> {
