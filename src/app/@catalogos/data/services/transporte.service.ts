...
export class TransporteService {
  ...
  private urlEndPoint: string = environment.UrlAguilaApi + "/api/transportes";
  ...
  private listaDatos = new BehaviorSubject<Transporte[]>([]);
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      map((response: any) => response.aguilaData as Transporte),
      catchError((e) => {
  ...
  setDatos(lista: Transporte[]) {
    this.listaDatos.next(lista);
  }

  getDatos(cargar: boolean = false): Observable<Transporte[]> {
  ...
  cargarPagina(noPagina: number = 1, fNombre: string = "") {
    if (this.validarPermiso('Consultar')) {
      return this.http.get(this.urlEndPoint +
        `?PageNumber=${noPagina}&nombre=${fNombre}`).pipe(
