...
export class TipoActivosService {
  ...
  private urlEndPoint: string = environment.UrlAguilaApi + "/api/tipoActivos";
  ...
  private listaDatos = new BehaviorSubject<TipoActivo[]>([]);
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      map((response: any) => response.aguilaData as TipoActivo),
      catchError((e) => {
  ...
  setDatos(lista: TipoActivo[]) {
    this.listaDatos.next(lista);
  }

  getDatos(cargar: boolean = false): Observable<TipoActivo[]> {
