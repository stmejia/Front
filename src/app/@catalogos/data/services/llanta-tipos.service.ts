import { LlantaTipo } from './../models/llantaTipo';
...
export class LlantaTiposService {

  private urlEndPoint: string = environment.UrlAguilaApi + "/api/llantaTipos";
  ...
  private listaDatos = new BehaviorSubject<LlantaTipo[]>([]);
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      map((response: any) => response.aguilaData as LlantaTipo),
      catchError((e) => {
  ...
  setDatos(lista: LlantaTipo[]) {
    this.listaDatos.next(lista);
  }

  getDatos(): Observable<LlantaTipo[]> {
