...
import { InventarioCategoria } from '../models/inventarioCategoria';
...
export class InventarioCategoriaService {
  ...
  /* ----- ----- Variables Genericas ----- ----- */
  private urlEndPoint: string = environment.UrlAguilaApi + "/api/invCategoria";
  ...
  private listaDatos = new BehaviorSubject<InventarioCategoria[]>([]);
  /* ----- ----- Fin ----- ----- */
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      first(),
      map((res: any) => res.aguilaData as InventarioCategoria),
      catchError((e) => {
  ...
  crear(item: InventarioCategoria) {
  ...
  modificar(item: InventarioCategoria) {
  ...
  setDatos(lista: InventarioCategoria[]) {
    this.listaDatos.next(lista);
  }

  getDatos(): Observable<InventarioCategoria[]> {
    ...
    if (this.validarPermiso('Consultar')) {
      this.http.get(this.urlEndPoint +
        `?PageNumber=${noPagina}&idEmpresa=${idEmpresa}`).pipe(
          ...
          map((response: any) => response.aguilaData as InventarioCategoria[]),
