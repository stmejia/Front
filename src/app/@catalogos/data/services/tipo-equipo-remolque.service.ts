import { TipoEquipoRemolque } from './../models/tipoEquipoRemolque';
...
export class TipoEquipoRemolqueService {

  private urlEndPoint: string = environment.UrlAguilaApi + "/api/tipoEquipoRemolque";
  ...
  private listaDatos = new BehaviorSubject<TipoEquipoRemolque[]>([]);
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      map((response: any) => response.aguilaData as TipoEquipoRemolque),
      catchError((e) => {
  ...
  modificar(item: TipoEquipoRemolque) {
  ...
  setDatos(lista: TipoEquipoRemolque[]) {
    this.listaDatos.next(lista);
  }

  getDatos(cargar: boolean = false): Observable<TipoEquipoRemolque[]> {
