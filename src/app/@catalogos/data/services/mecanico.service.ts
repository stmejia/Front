...
export class MecanicoService {
  ...
  private urlEndPoint: string = environment.UrlAguilaApi + "/api/mecanicos";
  ...
  private listaDatos = new BehaviorSubject<Mecanico[]>([]);
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      first(),
      map((res: any) => res.aguilaData as Mecanico),
      catchError((e) => {
  ...
  setDatos(lista: Mecanico[]) {
    this.listaDatos.next(lista);
  }

  getDatos(): Observable<Mecanico[]> {
    ...
    if (this.validarPermiso('Consultar')) {
      this.http.get(this.urlEndPoint +
        `?PageNumber=${noPagina}`).pipe(
          ...
          map((response: any) => response.aguilaData as Mecanico[]),
          tap((res) => this.setDatos(res)),
          catchError((e) => {
            this.sweetService.sweet_Error(e);
            return [];
          })
        ).subscribe((res) => {this.sweetService.sweet_notificacion("Listo", 2000); console.log(res)});
    } else {
