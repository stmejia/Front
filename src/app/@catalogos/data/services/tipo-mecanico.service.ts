...
import { TipoMecanico } from '../models/tipoMecanico';
...
export class TipoMecanicoService {

  private urlEndPoint: string = environment.UrlAguilaApi + "/api/tipoMecanicos";
  ...
  private listaDatos = new BehaviorSubject<TipoMecanico[]>([]);
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      map((response: any) => response.aguilaData as TipoMecanico),
      catchError((e) => {
  ...
  setDatos(lista: TipoMecanico[]) {
    this.listaDatos.next(lista);
  }

  getDatos(cargar: boolean = false): Observable<TipoMecanico[]> {
  ...
  cargarPagina(noPagina: number = 1, filtroNombre = "") {
    if (this.validarPermiso('Consultar')) {
      return this.http.get(this.urlEndPoint +
        `?PageNumber=${noPagina}&nombre=${filtroNombre}`).pipe(
