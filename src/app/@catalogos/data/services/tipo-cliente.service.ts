...
export class TipoClienteService {
  ...
  private urlEndPoint: string = environment.UrlAguilaApi + "/api/tipoClientes";
  ...
  private listaDatos = new BehaviorSubject<TipoCliente[]>([]);
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      map((response: any) => response.aguilaData as TipoCliente),
      catchError((e) => {
  ...
  setDatos(lista: TipoCliente[]) {
    this.listaDatos.next(lista);
  }

  getDatos(cargar: boolean = false): Observable<TipoCliente[]> {
