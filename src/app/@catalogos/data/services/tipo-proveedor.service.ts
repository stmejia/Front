...
export class TipoProveedorService {
  ...
  private urlEndPoint: string = environment.UrlAguilaApi + "/api/tipoProveedores";
  ...
  private listaDatos = new BehaviorSubject<TipoProveedor[]>([]);
  ...
  getId(id: number) {
    return this.http.get(this.urlEndPoint + "/" + id).pipe(
      map((response: any) => response.aguilaData as TipoProveedor),
      catchError((e) => {
  ...
  setDatos(lista: TipoProveedor[]) {
    this.listaDatos.next(lista);
  }

  getDatos(cargar: boolean = false): Observable<TipoProveedor[]> {
  ...
  cargarPagina(noItems: number = 20, noPagina: number = 1, filtro: string = "") {
    if (this.validarPermiso('Consultar')) {
      return this.http.get(this.urlEndPoint +
        `?descripcion=${filtro}&PageSize=${noItems}&PageNumber=${noPagina}`).pipe(
