  ...
  async imprimirCondicion(id: number) {
    ...
    this.getId(id).subscribe(async (res) => {
      try {
        ...
        pdf.add({ text: "CONDICIÓN EQUIPO DE REMOLQUE", bold: true, alignment: "center", fontSize: 12, margin: [0, -10, 0, 5] });
        ...
        let detalle = [
          {
            text: "Detalle De Condición", bold: true, alignment: "center", fontSize: 12, margin: [0, 5, 0, 5]
          },
          {
        ...
        pdf.add(informacion);
        pdf.add(detalle);
        pdf.add({ text: "Condición De Llantas", alignment: "center", bold: true, fontSize: 12, margin: [0, 5, 0, 5] });
        pdf.add(tablaLlantas);

        if (res.condicionesLlantasRepuesto.length > 0) {
          pdf.add({ text: "Condición Llantas De Repuesto", alignment: "center", bold: true, fontSize: 12, margin: [0, 5, 0, 5] });
