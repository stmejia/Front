...
import { SolicitudesReservasComponent } from '../../components/altas/solicitudes-reservas/solicitudes-reservas.component';
...
export class ControlEquipoService {

  private urlEndPoint: string = environment.UrlAguilaApi + "/api/controlEquipo";
  ...
  private listaDatosReporteMovimientos = new BehaviorSubject<EquipoRemolque[]>([]);
  ...
  cargarPaginaReportes(filtros: QueryFilter[]): void { //Inventario
    if (!this.validarPermiso('reporteInventario')) {
      this.errorPermiso("Reporte Inventario");
      return;
    }
    this.sweetService.sweet_carga("Cargando Información");
    let filter = "?";
  ...
  getEstados(): Observable<Estados[]> {
    if (!this.validarPermiso('Consultar')) {
      this.errorPermiso("Consultar");
      return throwError("Error Permiso");
    }
  ...
  setIngresoSalida(ingresoSalida: any) {
    if (!this.validarPermiso('IngresoSalida')) {
      this.errorPermiso("Ingreso-Salida");
      return throwError("Error Permiso");
    }
  ...
  reservarEquipo(reserva: any) {
    if (!this.validarPermiso('Reservar')) {
      this.errorPermiso("Reservar");
      return throwError("Error De Permiso");
    }
    this.sweetService.sweet_carga("Espere");
  ...
  evniarReparacionEquipo(movimiento: any) {
  ...
  evniarRentaInternaEquipo(movimiento: any) {
    if (!this.validarPermiso('RentaInterna')) {
      this.errorPermiso("Renta Interna");
      return throwError("Error De Permiso");
    }
    this.sweetService.sweet_carga("Espere");
  ...
  preReservarEquipo(movimiento: any) {
    if (!this.validarPermiso('PreReserva')) {
      this.errorPermiso("PreReserva");
      return throwError("Error De Permiso");
    }
    this.sweetService.sweet_carga("Espere");
    return this.http.post(this.urlEndPoint + '/preReserva', movimiento).pipe(
      first(),
      map((r: AguilaResponse<any>) => r.aguilaData),
      catchError((e) => {
        this.sweetService.sweet_Error(e);
  ...
  seleccionarSolicitudAltas() {
    return this.modal.open(SolicitudesReservasComponent).afterClosed();
  }
  ...
  getTiposEquipos() {
    return this.equipoService.getTiposEquiposRemolque();
  }

  getTiposEquiposValue() {
    return this.equipoService.getTiposEquiposRemolqueValue();
  }

  getRecursoEquipos() {
    return this.equipoService.getRecurso();
  }
}
