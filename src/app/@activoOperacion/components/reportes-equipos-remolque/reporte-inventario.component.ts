...
@Component({
  selector: 'app-reportes-equipos-remolque',
  templateUrl: './reporte-inventario.component.html',
})
export class ReporteInventarioEquiposRemolqueComponent implements OnInit {

  cargandoDatos: boolean = true;
  private queryFilters: QueryFilter[] = [];
  private listaEstados: Estados[] = [];
  private listaDatos: EquipoRemolque[] = [];
  ...
  constructor(private serviceComponent: ControlEquipoService, private sweetService: SweetService,
  ...
  cargarComponent() {
    this.serviceComponent.setConfiguracionComponent({
      header: {
        titulo: 'Inventario Equipos De Remolque',
        opciones: [
          {
  ...
  setFiltrosComponent() {
    let tipoVehiculos = this.serviceComponent.getTiposEquiposValue();
    let filtros: FiltrosC[] = [];
    let f: FiltrosC = {
      activo: true,
      nombre: "Tipo Equipo Remolque",
  ...
  generarDataGraficas(data) {
    ...
    */
    ...
    for (let tipo in datosXEquipo.datos) {
      ...
      labelsEstado.forEach(estado => {
        let noItemsEstado = this.listaDatos.filter(equipo => (equipo.tipoEquipoRemolque.prefijo.toUpperCase().trim() == tipo)
          && (equipo.activoOperacion.movimientoActual.estado.nombre.toUpperCase().trim() == estado)).length;
        dataSeries.data.push(noItemsEstado);
      });
  ...
  generarPdf(evento) {
    let titulos: TitulosReporte = {
      ...
      subTitulos: [
        { text: "Inventario De Equipos", bold: true, size: 18 }
      ]
    }
  ...
  generarExcel(evento) {
    let titulos: TitulosReporte = {
      ...
      subTitulos: [
        { text: "Inventario De Equipos", bold: true, size: 14 },
      ]
    }
      ...
      if (f.parametro !== "") {
        switch (f.filtro) {
          case "idTipoEquipoRemolque":
            filtrosAplicados += ` -Tipo De Vehiculo: ${this.serviceComponent.getTiposEquiposValue().find(t => t.id.toString() == f.parametro).prefijo}`;
            break;
          case "flota":
  ...
  getDatosXEquipos() {
    ...
    this.listaDatos.forEach(equipo => {
      let tpEquipo = equipo.tipoEquipoRemolque.prefijo.toUpperCase().trim();
