...
export class ReporteMovimientosEquiposRemolqueComponent implements OnInit {
  ...
  private columnas: ColumnaTabla[] = [
    ...
    { titulo: 'Placa', target: ['activoOperacion', 'equipoRemolque', 'placa'], tipo: 'texto', aligment: 'left', visible: true },
  ...
  header: ItemHeaderComponent = {
    titulo: 'Movimientos De Equipo Remolque',
    opciones: [
      {
  ...
  setFiltrosComponent() {
    let tipoEquipo = this.serviceComponent.getTiposEquiposValue();
  ...
  generarPdf(evento) {
    let titulos: TitulosReporte = {
      ...
      subTitulos: [
        { text: "Movimientos De Equipo", bold: true, size: 18 }
      ]
    }
  ...
  generarExcel(evento) {
    let titulos: TitulosReporte = {
      ...
      subTitulos: [
        { text: "Movimientos De Equipo", bold: true, size: 14 },
      ]
    }
