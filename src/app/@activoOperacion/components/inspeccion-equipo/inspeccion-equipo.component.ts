...
@Component({
  ...
  templateUrl: './inspeccion-equipo.component.html',
  styleUrls: ['./inspeccion-equipo.component.css'],
  providers: [{
    provide: STEPPER_GLOBAL_OPTIONS, useValue: { showError: true }
  }]
})
export class InspeccionEquipoComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private service: CondicionEquipoService,
  ...
  cargarComponent() {
    ...
    this.service.setConfiguracionComponent({
      header: {
        titulo: "Inspección De Equipo Remolque",
        opciones: opt
      },
      isModal: false,
    });
      ...
      this.filtrosEquipo.push(
        {
          ...
          valores: res[0].aguilaData.filter(te => { if (te.prefijo.startsWith("PL") || te.prefijo.startsWith("CH")) return true }).map(te => { return { nombre: te.prefijo + "-" + te.descripcion, valor: te.id } })
        }
      );
  ...
  configurarFormulario() {
    this.form = this.formBuilder.group({
      ...
      guardaFangosG: ["", [Validators.required]],
      guardaFangosI: ["", [Validators.required]],

      cintaReflectivaLat: ["", [Validators.required]],
      cintaReflectivaFront: ["", [Validators.required]],
      cintaReflectivaTra: ["", [Validators.required]],

      fricciones: ["", [Validators.required]],
      friccionesLlantas: [""],

      bumper: ["", [Validators.required]],
      manitas1: ["", [Validators.required]],
      manitas2: ["", [Validators.required]],
      patas: ["", [Validators.required]],
      ganchos: ["", [Validators.required]],

      balancines: ["", [Validators.required]],
      hojasResortes: ["", [Validators.required]],
    ...
    this.formCondicionActivo = this.formBuilder.group({
      ...
      tipoCondicion: ["equipo", [Validators.required]],
  ...
  cargarDatos() {
    forkJoin([
      this.service.getEstadosCondiciones()
