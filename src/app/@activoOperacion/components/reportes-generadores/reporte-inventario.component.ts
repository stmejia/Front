...
export class ReporteInventarioGeneradorComponent implements OnInit {

  cargandoDatos: boolean = true;
  private queryFilters: QueryFilter[] = [];
  private listaEstados: Estados[] = [];
  private listaDatos: Generador[] = [];
  ...
  cargarComponent() {
    this.serviceComponent.setConfiguracionComponent({
      header: {
        titulo: 'Inventario De Generadores',
        opciones: [
          {
  ...
  setFiltrosComponent() {
    let tipoVehiculos = this.serviceComponent.getTiposGeneradoresValue();
    let filtros: FiltrosC[] = [];
    let f: FiltrosC = {
      activo: true,
      nombre: "Tipo De Generador",
  ...
  generarDataGraficas(data) {
    ...
    for (let tipo in datosXEquipo.datos) {
      ...
      labelsEstado.forEach(estado => {
        let noItemsEstado = this.listaDatos.filter(equipo => (equipo.tipoGenerador.prefijo.toUpperCase().trim() == tipo)
          && (equipo.activoOperacion.movimientoActual.estado.nombre.toUpperCase().trim() == estado)).length;
        dataSeries.data.push(noItemsEstado);
      });
  ...
  generarPdf(evento) {
    let titulos: TitulosReporte = {
      ...
      subTitulos: [
        { text: "Inventario De Generadores", bold: true, size: 18 }
      ]
    }
  ...
  generarExcel(evento) {
    let titulos: TitulosReporte = {
      ...
      subTitulos: [
        { text: "Inventario De Generadores", bold: true, size: 14 },
      ]
    }
      ...
      if (f.parametro !== "") {
        switch (f.filtro) {
          case "idTipoGenerador":
            filtrosAplicados += ` -Tipo De Generador: ${this.serviceComponent.getTiposGeneradoresValue().find(t => t.id.toString() == f.parametro).prefijo}`;
            break;
          case "flota":
  ...
  getDatosXEquipos() {
    ...
    this.listaDatos.forEach(equipo => {
      let tpEquipo = equipo.tipoGenerador.prefijo.toUpperCase().trim();
